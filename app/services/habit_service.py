...
def complete_habit(
    ...
    return set_habit_completed_today(db, habit, True)
...
def update_habit(
    ...
    existing_habit.title = habit.name
    ...
    existing_habit.reminder = habit.reminder
    existing_habit.paused = habit.paused
...
def set_habit_completed_today(
    ...
    if done:
        ...
        if log:
            ...
            db.add(HabitLog(habit_id=habit.id, log_date=today, completed=True))
    else:
...
def update_habit_partial(
    ...
    done_today = data.pop("doneToday", None)

    if "name" in data:
        existing_habit.title = data.pop("name")
    ...
    if done_today is not None:
        return set_habit_completed_today(db, existing_habit, done_today)
