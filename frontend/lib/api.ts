 ...
 * expired, or otherwise rejected by the backend — the frontend has no way
 * to tell those apart. Rather than let every page render its own confusing
 * "Invalid token" error, wipe the bad token and send the user back to
 * /login once, from one place. Login/register calls pass `auth: false` and
 * are never affected by this — a wrong username/password there just
